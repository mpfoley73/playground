...
total_pop_plot <- function(subdiv_yyyy, block_yyyy, yyyy) {
  ...
  labs(
    ...
    subtitle = glue(
      "Cuyahoga County Subdivisions (sage) and Cleveland SPAs (green).")
  )
...
neigh_gt <- function(.rpt_group, .rpt_level) {
  ...
  by_subdiv <- 
    ...
    as_tibble() |>
    filter(rpt_group == .rpt_group, rpt_level == .rpt_level) |>
    mutate(SPA = if_else(SPA == "Cleveland", SPA, "Other")) |>
...
subdiv_long |>
  filter(rpt_group %in% c("Race/ethnicity")) |>
  mutate(SPA = if_else(SPANM == "Cleveland", SPANM, "Other Parts of Cuyahoga County")) |>
  summarize(.by = c(yr, SPA, rpt_level), value = sum(value)) |>
  mutate(.by = c(yr, SPA), pct = value / sum(value)) |>
  pivot_wider(names_from = yr, values_from = c(value, pct)) |>
  select(SPA, rpt_level, ends_with("2000"), ends_with("2010"), ends_with("2020"), 
         ends_with("2023")) |>
  mutate(across(starts_with("pct"), \(x) percent(x, 1))) |>
  janitor::adorn_totals(where = "row", fill = "", na.rm = TRUE, name = "County Total",
                        starts_with("value")) |>
  mutate(
    rpt_level = factor(rpt_level, levels = levels(subdiv_long$rpt_level)),
    SPA = factor(SPA, levels = c("Cleveland", "Other Parts of Cuyahoga County", "County Total"))
  ) |>
  arrange(SPA, rpt_level) |>
  group_by(SPA) |>
  gt::gt() |>
...
x <-
  neigh_long |>
  filter(rpt_group %in% c("Race/ethnicity", "Total")) |>
  mutate(.by = c(yr, SPA), pct = value / sum(value) * 2)
# neigh_long |> filter(rpt_group == "Total", yr == "2000", SPA == "Collinwood Nottingham")
x_tooltip <- 
  x |>
  select(-rpt_group) |>
  pivot_wider(names_from = rpt_level, values_from = c(value, pct)) |>
  mutate(tt = glue(
    "{SPA} | {yr}\n\n",
...
p <-
  ...
  geom_col_interactive(aes(fill = rpt_level, tooltip = tt)) + 
  facet_wrap(vars(SPA)) +
